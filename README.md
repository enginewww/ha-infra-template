...
### Заметка о конфигурации Ansible

В текущей конфигурации Ansible подключается к контейнерам по SSH.  
Каждый контейнер запускает SSH-сервер, а Ansible использует ключи для аутентификации.

> Примечание: альтернативно, можно использовать docker плагин (`community.docker.docker_container`) для управления контейнерами напрямую через Docker API, без необходимости запускать SSH внутри контейнеров. Но буду следовать ТЗ :)
...
- **1 узел управления Ansible:** контейнер для запуска плейбуков.
- **Мониторинг:** Grafana и Victoria Metrics для сбора и визуализации метрик.
